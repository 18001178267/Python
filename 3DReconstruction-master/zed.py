...
while not exit_signal:
    if zed.grab(runtime_parameters) == sl.ERROR_CODE.SUCCESS:
        ...
        if key == 1048689:  #'q' waitfor 'q' keytoexit
            cv2.destroyAllWindows()
            exit_signal = True
        elif key == 115: #key == 's':  # wait for 's' key to save and exit
            if not os.path.exists('zed_left_image'):
                os.makedirs('zed_left_image')
            else :
                pass
            zed_image_num +=1
            cv2.imwrite('./zed_left_image/'+str(zed_image_num).zfill(3)+'.jpg',left_image)
