...
def count(totalnum,totaldistance,velocity_):
    ...
    while 1:
        ...
        # print(now)
        now = float(now)
        v = 0.7*int((up+down)/2)-now
        velocity_.delete(0,'end')
        # velocity_.insert(0, str(float('%.2f' % v)))
        velocity_.insert(0, str(round(v)))
        totalnum.delete(0,'end')
        totalnum.insert(0,int((up+down)/2))
        totaldistance.delete(0,'end')
        totaldistance.insert(0,str(float('%.2f' % (0.7*int((up+down)/2)))))
        time.sleep(1)
        t = time.time()

def realtime(initialdistance,totalnum,threshold,real_distance):
    global up,down,array,judge
    compare = float(initialdistance.get())
    while 1:
        s = ser.read(3)
        s0='{:08b}'.format(s[0])
        s1='{:08b}'.format(s[1])
        ...
        if 100 > fin :
            ...
            if compare-fin < (-1.0*threshold) :
                ...
                down += 1
                compare = fin
            elif compare-fin > 1.0*threshold :
                judge.pop(0)
                judge.append(1)
                # print(np.sum(judge),len(judge))
                if np.sum(judge)==4:
                    up += 1
                    compare = float(initialdistance.get())
            else :
