...
image = cv2.imread('C:\\Users\\YFZX\\Desktop\\Python_code\\license_plate\\bullet_hole_3.jpg')
output=image.copy()
#print(image.shape)
...
#ret, thresh = cv2.threshold(gauss, 150, 255, cv2.THRESH_BINARY) # 阈值分割bullet_hole_2 NOO
ret, thresh = cv2.threshold(gauss, 70, 255, cv2.THRESH_BINARY) # 阈值分割bullet_hole_3
...
for c in contours:
    ...
    circle_r= int(w/2)
    #print(x,y,w,h)
    if  h >50 or w>50 or float(h/w)>1.3 or float(w/h)>1.3 or h <5 or w< 5 :      # 筛选边框
        continue
...
#cv2.imwrite('C:\\Users\\YFZX\\Desktop\\Python_code\\license_plate\\' + 'Result9.jpg',output)
