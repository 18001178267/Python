...
def fetch_data(sensor_num):                            #根据输入的传感器编号数量获取指定设备的X,Y,Z,T数据,并存入整体数组的相应INDEX位置（整体数组是嵌套数组）
    ...
    for i in range(1,(sensor_num+1)):
        ...
        start = datetime(2019, 11, 4, 22, 10,40) # 用指定日期时间创建datetime
...
def save_mysql_according_to_sensor_num(sensor_num):
    ...
    for ii in range(1,(sensor_num+1)):
        for dt in range(0,int(len( all_sensor_data['num_'+str(sensor_num)+'_x']))):
            ...
            #print(type(t[dt]))
            sql="INSERT IGNORE INTO imei"+lastimei[ii]+"(x,y,z,data_time) VALUES ("
            sql=sql+ all_sensor_data['num_'+str(ii)+'_x'][dt]+","+ all_sensor_data['num_'+str(ii)+'_y'][dt]+","+ all_sensor_data['num_'+str(ii)+'_z'][dt]+","+"'"+ all_sensor_data['num_'+str(ii)+'_t'][dt]+"'"+")"     #t[dt]两边要用单引号括起来！很重要！！！
            print(sql)
            cursor.execute(sql)
